...
namespace RTSEngine.Cameras
{
    public abstract class MainCameraZoomHandlerBase : MonoBehaviour, IMainCameraZoomHandler
    {
        ...
        private SmoothSpeedRange zoomSpeed = new SmoothSpeedRange { valueRange = new FloatRange(8f,15f), acceleration = 10.0f, deceleration = 5.0f };
        ...
        protected virtual void ApplyTransformZoom(Vector3 middleScreenTerrainPoint)
        {
            ...
            lastZoomValue = Mathf.MoveTowards(
                lastZoomValue,
                zoomValue,
                Time.deltaTime * (zoomValue == 0.0f ? zoomSpeed.deceleration : zoomSpeed.acceleration));
